...
from model_contrastiveloss import Siamese_net
import pandas as pd
...
def main():
    ...
    model.load_weights("F:/pc/change_detection_tf/check_points/contrastiveloss2/weights-improvement-03.hdf5")
    preds=[]
    trues=[]
    ...
    for i in range(test_set.shape[0]):
        ...
        maska,_ = model.predict([img1_1, img2_1])
        maskb,_ = model.predict([img1_2, img2_2])
        maskc,_ = model.predict([img1_3, img2_3])
        maskd,_ = model.predict([img1_4, img2_4])
    ...
    savepath='F:/pc/change_detection_tf/check_points/confusion_matrixs'
    if not os.path.exists(savepath):
        os.makedirs(savepath)
    table.to_csv(savepath + '/confusion_matrixs_contrastiveloss2.csv')
